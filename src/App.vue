...
export default {
  ...
  methods: {
    addTicker() {
      ...
      setInterval(async () => {
        const API_URL = `https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=1b0437818e0cb14a189da8a65b5647e6e7d3475766dd3cc04be40e28874d3459`;

        const response = await fetch(API_URL);
        const result = await response.json();

        this.findTicker(currentTicker).price =
    ...
    findTicker(requiredTicker) {
      return this.tickers.find((ticker) => ticker.name === requiredTicker.name);
    },
    normalizeGraph() {
